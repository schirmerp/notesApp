...
def get_notes(a):
    ...
    with open('notes.txt', 'r') as f:
        read = f.readlines()
        for line in read:
            p = r"(\d+\/\d+\/\d+)\:([\w\d\-\.\s]+)\:([\w\d\s\.*]+)"
            d = re.search(p, line)
            if a.lower() in d.groups():
                return d.groups()
            else:
                notes.append([d.group(1), d.group(2), d.group(3)])
    return notes
...
def add_notes():
       ...
       #Insert into the table
               ...
               mod('notes.txt', " ".join([f"{today}:", f"{notes_title}:", f"{notes}"]))
